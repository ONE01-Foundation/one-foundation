...
export function SectionAnonymously() {
  ...
  useEffect(() => {
    ...
    intervalRef.current = setInterval(() => {
      setActiveIndex((prev) => (prev + 1) % copy.anonymously.features.length);
      setProgress(0);
    }, 6000);
  ...
  }, [isInView, copy.anonymously.features.length]);
  ...
  const leftOrbs = copy.anonymously.features.slice(0, 2);
    ...
    >
      <div className="max-w-7xl mx-auto w-full">
        ...
        <div className={`grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12 items-start ${isRTL ? "lg:grid-flow-col-dense" : ""}`}>
          ...
          >
            <div className="h-96 w-full max-w-sm bg-neutral-200 dark:bg-neutral-800 rounded-2xl flex items-center justify-center">
          ...
          <div className={`flex flex-col gap-6 items-center lg:items-start ${isRTL ? "lg:col-start-1" : "lg:col-start-3"}`}>
            <FeatureCarousel
              features={copy.anonymously.features}
              sectionId="anonymously"
              autoRotate={false}
              layout="split"
              isRTL={isRTL}
